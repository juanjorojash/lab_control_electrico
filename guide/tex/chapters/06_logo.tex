...
\begin{itemize}
	\item 2 motores trifásicos 1/2 hp de 3 puntas.
...
\begin{itemize}
	\item Se necesita un alternador de bombas  para el llenado de un tanque de almacenamiento. El sistema cuenta con dos botoneras para el arranque y pare general del sistema.
	\item El taque posee tres niveles: nivel alto (NA), nivel bajo (NB) y nivel crítico (NC).
	\item Además, se tienen dos bombas: B1 y B2
	\item Cuando el agua activa NB la bomba B1 se activa y se apaga hasta alcanzar el nivel NA.
	\item La próxima vez que se active el sensor NB se activa la segunda bomba B2 y se apaga hasta alcanzar NA.
	\item Este ciclo se repite hasta que se presione el apagado general del sistema o se active una de las dos sobrecargas.
	\item Si el nivel crítico se presiona NC se encienden ambas bombas, además el sistema debe  recordar el orden de la alternancia.
	\item El software LOGO! Soft Comfort deberá mostrar en pantalla el estado de la bombas y la etapa lógica en que se encuentra el sistema.
\end{itemize}
...
\begin{itemize}
	\item El diagrama lógico de la solución.
	\item El diagrama de la implementación con sus  ecuaciones lógicas.
	\item El diagrama de conexión eléctrico.
\end{itemize}
...
\begin{itemize}
	\item botón del arranque motor A
	\item botón de paro del motor A
	\item presostato de confirmación del motor A
	\item botón del arranque motor B
	\item botón de paro del motor B
	\item presostato de confirmación del motor B
\end{itemize}

Las salidas del controlador son las señales a los contactores de la moto-bomba A y la moto-bomba B.
