...
%     \tikzset{sig/.style={muxdemux, muxdemux def={Lh=4, Rh=4, NL=2, NB=0, NR=0, w=2}}}
%     \centering
%     \begin{circuitikz} 
%         \draw 
%         (0,3.5) 
...
%         node[dig] (m){\rotatebox{90}{\small digital IO}};
%         (0,6)
%         node[dig] (s){\rotatebox{90}{\small signal gen}};
%         \draw (m.blpin 1) node[above left]{\small DIO2};
...
%         ;        
%         \draw[dashed,blue]
%         (-2.5,6) -- (1,6)node[midway, below]{ARDUINO UNO} -- (1,-5.5) -- (-2.5,-5.5) -- cycle;
%     \end{circuitikz}
