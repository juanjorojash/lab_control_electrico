\chapter{Programación de variadores de frecuencia y arrancadores suaves.}
...
Implemente un circuito de control y potencia para que el motor arranque en dos sentidos, para esto puede usar como referencia la sección 16 de ejemplos. 
 ...
 \begin{figure}[H]
    \centering
		...
		(0,-3.5) 
		to [contact NO={info={$F$}}] ++ (3,0) -- ++ (0,-1.2) node[above right]{1}
		(0,-4.2)
		to [contact NO={info={$R$}}] ++ (3,0)
		(0,-5) -- ++ (3,0) node[above,xshift=-0.9cm]{$A_1$} node[above,xshift=0.9cm]{$A_2$} node[draw,fill=white,minimum width=1.2cm,minimum height=1.2cm] {} -- ++ (2,0)
		;
        \ladderrungend{6}
        \ladderpowerrails
        \end{tikzpicture}
		\caption{Circuito de control: arrancador suave}
 \end{figure}

\begin{figure}[H]
    \centering
    \begin{circuitikz}[cute inductors,scale=0.7]
    \node[esourceshape,name=motor] at (14,-2){};
    \draw
    (0,0) node[left]{$L_1$}
    ...
    to[short,-*]
    (L22)
    (L13) -- ++ (1.2,0) coordinate(SSin1)
    (L23) -- ++ (1.2,0) coordinate(SSin2)
    (L33) -- ++ (1.2,0) coordinate(SSin3)
    (SSin2) ++ (1.4,0) node[draw,minimum width=2.0cm,minimum height=4.0cm] (ss) {}
    (SSin1) -- (ss.west |- SSin1)
    (SSin2) -- (ss.west)
    (SSin3) -- (ss.west |- SSin3)
    (ss.east |- SSin1) coordinate(SSout1)
    (ss.east) coordinate(SSout2)
    (ss.east |- SSin3) coordinate(SSout3)
    (SSout1) to [wfuse,l=$S_{c}$] ++ (2,0) -- (motor.120)
    (SSout2) to [wfuse,l=$S_{c}$] ++ (2,0) -- (motor.180)
    (SSout3) to [wfuse,l=$S_{c}$] ++ (2,0) -- (motor.240)
    (motor.center) node{$M$}
    ;
    \end{circuitikz}
    \caption{Circuito de potencia: arrancador suave reversible}
\end{figure}
